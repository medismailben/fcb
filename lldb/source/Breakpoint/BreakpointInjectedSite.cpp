...
bool BreakpointInjectedSite::BuildConditionExpression(void) {
  ...
  StackFrameSP frame_sp =
      std::make_shared<StackFrame>(thread_sp, frame_idx, concrete_frame_idx,
                                   cfa, cfa_is_valid, pc, frame_kind,
                                   zeroth_frame, &sc);
...
bool BreakpointInjectedSite::GatherArgumentsMetadata() {
  ...
  for (ExpressionVariableSP expr_var : members.Variables()) {
    ...
    VariableMetadata metadata(expr_var->GetName().GetCString(),
                              size.getValue(), llvm_data, addr_size);
...
bool BreakpointInjectedSite::CreateArgumentsStructure() {
  ...
  auto utility_fn_or_error = m_target_sp->CreateUtilityFunction(expr, name, eLanguageTypeC, m_owner_exe_ctx);
