...
//      bool name_is_mangled = false;
      ...
      const Symbol symbol(symID, name.c_str(), type, external,
                          is_debug, is_trampoline, is_artificial, section_sp, 0,
                          m_size, size_is_valid, contains_linker_annotions,
                          flags);
...
void ObjectFileTrampoline::Dump(Stream *strm) {
  ...
  if (module_sp) {
    ...
    if (sections)
      sections->Dump(strm->AsRawOstream(), strm->GetIndentLevel() + 2, nullptr, true, UINT32_MAX);
